Ext.define(enumPageInfo.injectionStep13.nameSpace, {
    alias: [enumPageInfo.injectionStep13.nameSpace],
    config: {
        mainPanel: null
    },
    name: enumPageInfo.injectionStep13.name,
    isPainted: false,
    isCorrect: false,
    ...
    createPanel: function () {
        ...
        try {
            mainPanel = Ext.create('Ext.Container', {
                ...
                hidden: true,
                html: '<div class="main">' +
                      '<div class="injection_step injection_step_13 fixed_height">' +
                        ...
                        '<div class="tips" id="div_' + me.name + '_Tips">' +
                          '<p id="p_' + me.name + '_Tips">Insert the Glucagon solution</p>' +
                        ...
                        '<div class="act_btn">' +
                            ...
                            '</a>' +
                        '</div>' +
                        '<div class="arrow_left" id="div_' + me.name + '_ArrowLeft"><img src="img/vp_arrowLeft.png" height="66" alt=""/></div>' +
                      '</div>' +
                    '</div>',
                listeners: {
    ...
    touchMoveEvent: function (mouseObj, dragParameters) {
        ...
        try {
            ...
            if ((me.firstClientX - mouseObj.clientX) / userData.getValue(enumLocalStorageItem.bodyZoom) < 0) {
        ...
        try {
            if (me.isCorrect) {
                ...
                window.setTimeout(function () {
                    Base.gotoPage(enumPageInfo.injectionStep14);
                }, 1000);
            }
            else {
                $('#div_' + me.name + '_Tips').css('visibility', 'visible');
    ...
    needleDragEvent: function () {
        ...
        try {
            ...
            var distance = document.getElementById('img_' + me.name + '_Syringe').offsetWidth / 4 - 2 * userData.getValue(enumLocalStorageItem.imageZoom);
