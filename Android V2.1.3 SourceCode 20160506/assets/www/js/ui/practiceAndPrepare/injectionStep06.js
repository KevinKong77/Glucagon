Ext.define(enumPageInfo.injectionStep06.nameSpace, {
    alias: [enumPageInfo.injectionStep06.nameSpace],
    config: {
        mainPanel: null
    },
    name: enumPageInfo.injectionStep06.name,
    isPainted: false,
    isCorrect: false,
    ...
    createPanel: function () {
        ...
        try {
            mainPanel = Ext.create('Ext.Container', {
                ...
                hidden: true,
                html: '<div class="main">'
                            + '<div class="injection_step injection_step_6 fixed_height">'
                                ...
                                + '<div class="tips" id="div_' + me.name + '_Tips">'
                                    + '<p id="p_' + me.name + '_Tips">Remove the syringe from the vial</p>'
    ...
    touchMoveEvent: function (mouseObj, dragParameters) {
        ...
        try {
            ...
            var newBottom = dragParameters.originBottom + (dragParameters.originY - mouseObj.clientY) / userData.getValue(enumLocalStorageItem.bodyZoom);

            if (me.firstClientY - mouseObj.clientY < 0) {
                newBottom = me.needleOriginBottom;
            }

            if (newBottom - me.needleOriginBottom > dragParameters.distance) {
                newBottom = dragParameters.distance + me.needleOriginBottom;
            }

            $('#div_' + me.name + '_Syringe').css('bottom', newBottom - (me.needleOriginBottom - me.syringeOriginBottom) + 'px');
    ...
    needleDragEvent: function () {
        ...
        try {
            ...
            var distance = document.body.scrollHeight / userData.getValue(enumLocalStorageItem.bodyZoom) - me.syringeOriginBottom - targetPos;
            distance += 40 * userData.getValue(enumLocalStorageItem.imageZoom);
